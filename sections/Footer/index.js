...
const Footer = () => {
    ...
    return (
        <div style={{backgroundColor:'#0D121D'}}>
            <Container>
                <div className='relative w-full mt-14 md:pb-5 md:pt-28'>
                    <div className='flex flex-col w-full md:flex-row'>
                        <Logo />
                        <div className="flex flex-row flex-wrap w-full mt-8 text-white md:justify-end md:space-x-36 pb-28 md:mt-0">
                            {
                                links.map((link, index)=> {
                                    return (
                                        <div key={link.title+index} className="flex flex-col flex-wrap w-1/2 mt-4 md:w-auto ">
                                        <p className='pb-3 font-bold font-morgan text-18px'>{link.title}</p>
                                        <ul className="flex flex-col space-y-2">
                                            {link.subtitles.map((item, index)=> {
                                                return (
                                                    item.scroll ? <ScrollLink key={item.title + index} className="relative inline-block mt-4 mr-6 transition-all duration-75 cursor-pointer hover:text-white text-black-150 lg:mt-0 text-32px lg:text-14px" key={item.title + index} href={item.link} to={item.link.toLowerCase().replace('#', '')} spy={true} smooth={true}>{item.title}</ScrollLink> : <Link key={item.title + index} href={item.link}><a rel="noreferrer" className="relative inline-block mt-4 mr-6 transition-all duration-75 cursor-pointer text-black-150 lg:mt-0 text-32px lg:text-14px hover:text-white">{item.title}</a></Link>
                                                )
                                            })}
                                        </ul>
                                        </div>
                    ...
                    <div className='flex flex-row w-full font-semibold text-12px' style={{color: '#48536B'}}>
                        <div className="w-1/2">
                            <p>Copyright 2021 Bit Rivals. All rights reserved worldwide.</p>
                        </div>
                        <div className='flex flex-row w-full space-x-2 md:justify-end md:space-x-10'>
...
const Logo = () => {
    return (                  
        <Link href="/">
            <a rel="noreferrer" className="relative h-12 top-2">
                <svg width="152" height="60" viewBox="0 0 112 30" fill="none" xmlns="http://www.w3.org/2000/svg">
